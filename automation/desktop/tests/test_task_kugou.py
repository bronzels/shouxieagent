...
def test_run_done_when_agent_signals_finished():
    ...
    inp = MagicMock()
    task = KugouTask(win, agent, inp, add_hours=14, max_rounds=5)
    result = task.run()
...
def test_run_hits_max_rounds():
    ...
    inp = MagicMock()
    task = KugouTask(win, agent, inp, add_hours=14, max_rounds=3)
    result = task.run()
...
def test_each_round_activates_and_grabs():
    ...
    agent.step.return_value = {"type": "finished"}
    inp = MagicMock()
    KugouTask(win, agent, inp, add_hours=14, max_rounds=5).run()
